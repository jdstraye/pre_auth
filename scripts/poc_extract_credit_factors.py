...
def parse_count_amount_pair(s):
    ...
    This function looks for explicit token pairs around a slash and decides which side
    is count vs amount using presence of '$' or magnitude heuristics (counts usually <= 500).
    """
    ...
    if re.search(r"/\s*(mo|month|yr|year)\b", s_norm, re.I):
        return (None, None)
    # find all token pairs around a slash of form token1 / token2
    ...
    for left, right in pairs:
        ...
        if rnum <= 500 and lnum > 1000:
            return (rnum, lnum)
        # fallback: if left token has no comma but right has comma, prefer right as amount
        if ',' in right and ',' not in left:
            return (lnum, rnum)
        if ',' in left and ',' not in right:
            return (rnum, lnum)
        # otherwise, if left looks like an integer small and right similarly, assume left=count
        if lnum <= 500:
            return (lnum, rnum)
        if rnum <= 500:
            return (rnum, lnum)
        # final fallback: pick the pair and treat left as count
        return (lnum, rnum)
    return (None, None)
...
def ocr_regions_on_page(page, regions, scale=3, ocr_lang=None):
    ...
    for r in regions:
        ...
        try:
            data = pytesseract.image_to_data(crop, output_type=pytesseract.Output.DICT, lang=ocr_lang) if pytesseract else None
            tokens = []
            if data:
                for i, t in enumerate(data['text']):
                    if not t or not t.strip():
                        continue
                    lx = cx0 + data['left'][i]; ty = cy0 + data['top'][i]
                    w = data['width'][i]; hgt = data['height'][i]
                    page_scale_x = rect.width / pix.width
                    page_scale_y = rect.height / pix.height
                    # map token bbox back to page coords
                    tok_page_bbox = (rect.x0 + lx / (pix.width / rect.width), rect.y0 + ty / (pix.height / rect.height), rect.x0 + (lx+w) / (pix.width / rect.width), rect.y0 + (ty+hgt) / (pix.height / rect.height))
...
# Allow optional override of canonicals from human-labeled annotations
try:
    import json
    canon_path = ROOT / 'data' / 'label_canonicals.json'
...
# If label-driven canonicals exist, prefer them over hardcoded extras
try:
    ...
    if lab_path.exists():
        ...
        for k,v in _lc.items():
            if isinstance(v, (list, tuple)) and len(v) == 3:
                # ensure we add the filtered canonical as the primary canonical
                CANONICAL[k] = tuple(v)
except Exception:
    pass

# Tunable parameters for candidate scoring and classifier behavior (defined early to be available everywhere)
CANDIDATE_OVERLAP_FRAC = 0.66
UNIQ_SCORE_MULTIPLIER = 8
NUM_MATCH_WEIGHT = 140
AMBIGUITY_MARGIN = 4
CONV_PEAK_THRESH_DEFAULT = 12
CONV_RATIO_THRESH_DEFAULT = 1.25
# Set black threshold to include darker greys commonly used as 'black' text in PDFs
SPAN_BLACK_V_THRESH = 0.35
# Minimum saturation threshold for considering colored categories
SPAN_SAT_MIN = 0.05
# Weight for n-gram (2-gram) matches between phrase and candidate line
NGRAM_WEIGHT = 10
...
def find_credit_factors_region(doc, anchor='Credit Factors', max_pages=3):
    ...
    (page_index, blocks_to_scan) tuples covering the region. It will scan up to max_pages
    from the start and stop early if it sees likely section terminators.
    """
    start = None
    ...
    for p in range(start, min(start + max_pages, len(doc))):
        ...
        # heuristics: include consecutive blocks until we see a stop marker
        blocks_to_scan = []
        stop_here = False
        for b in blocks:
            ...
            if stop_here:
                break
        regions.append(blocks_to_scan)
        if stop_here:
            break
    # return list with start page and region blocks per page
    return list(enumerate(regions, start=start))
...
def span_color_hex(spans):
    ...
    for s in spans:
        col = s.get('color')
        if col:
            # convert various color encodings into 0-255 RGB tuple
            rgb = None
    ...
    # If any colored tokens exist, average only those
    if colored:
...
def detect_table_columns(page, min_count=2, bucket=12):
    ...
    for d in draws:
        ...
        w = rect.x1 - rect.x0
        h = rect.y1 - rect.y0
        # skip very small or very large rects
        if w < 6 or h < 6 or w > 800 or h > 400:
            continue
        centers.append((rect.x0 + rect.x1) / 2.0)
    if not centers:
        return []
    # bucket centers into approximate column positions
    buckets = {}
    for c in centers:
        key = int(c // bucket)
...
def raster_sample_color_near_line(page, line_bbox, zoom=4, debug_prefix=None, expected_color=None):
    ...
    This function is retained as a no-op stub for backward compatibility and will
    always return (None, None).
    """
    return None, None
...
def _median_around(img_arr, cx, cy, size=5):
    ...
    if patch.size == 0:
        return None
    med = tuple(map(int, np.median(patch.reshape(-1,3), axis=0)))
    return med
...
def _local_left_strip_conv_search(page, phrase, expected_color=None, scale=8, strip_w_px=30, conv_kernel=7, conv_threshold=20):
    ...
    try:
        ...
        return None
    img_arr = np.array(Image.frombytes('RGB', [pix.width, pix.height], pix.samples)).astype(int)
    ...
    s_x = pix.width / rect.width
    s_y = pix.height / rect.height
    for b in td.get('blocks', []):
        for ln in b.get('lines', []):
            line_text = ''.join([s.get('text','') for s in ln.get('spans', [])]).strip()
            if not line_text:
                continue
            if phrase.lower() not in line_text.lower() and not any(tok.lower() in phrase.lower() or phrase.lower() in tok.lower() for tok in line_text.split()):
                continue
            lx0, ly0, lx1, ly1 = ln.get('bbox')
            # convert to pixel coords
            px0 = max(0, int((lx0 - rect.x0) * s_x))
            py0 = max(0, int((ly0 - rect.y0) * s_y))
            px1 = min(pix.width, int((lx1 - rect.x0) * s_x))
            py1 = min(pix.height, int((ly1 - rect.y0) * s_y))
            # define a narrow strip left of the line
            strip_w = max(8, int(strip_w_px * (s_x/3.0)))
            ...
            if sx1 <= sx0 or sy1 <= sy0:
                continue
            crop = img_arr[sy0:sy1, sx0:sx1, :]
            if crop.size == 0:
                continue
            # compute greenness map
            g = crop[:,:,1].astype(int); r = crop[:,:,0].astype(int); b = crop[:,:,2].astype(int)
            greenness = (2*g - r - b)
            greenness = np.maximum(greenness, 0).astype(float)
            try:
                import scipy.ndimage as _nd
                sm = _nd.uniform_filter(greenness, size=conv_kernel)
            except Exception:
                # fallback simple mean filter
                sm = greenness
            # try greenness peak first
            maxv = sm.max()
            found_median = None
            if maxv >= conv_threshold:
                ...
                # prefer nonwhite pixels
                sample_sel = sample[sample.sum(axis=1) < 740]
                if sample_sel.size == 0:
                    sample_sel = sample
                median = tuple(map(int, np.median(sample_sel, axis=0)))
                if map_color_to_cat(median) == expected_color:
                    found_median = median
            # if greenness peak didn't find anything, try canonical-proximity seeding
            if found_median is None and target_arr.size:
                flat = crop.reshape(-1,3)
                # compute min distance to any canonical per pixel
                d = np.sum((flat[:, None, :] - target_arr[None, :, :])**2, axis=2)
                min_d = d.min(axis=1)
                best_idx = np.argmin(min_d)
                best_d = min_d[best_idx]
                # allow a reasonably loose threshold for pale/anti-aliased markers
                if best_d < 120000:
    ...
    for p in range(len(pdf_doc)):
        ...
        if phrase.lower() in text.lower():
            ...
            for i, txt in enumerate(data['text']):
                t = (txt or '').strip()
                if not t: continue
                norm = ''.join(ch.lower() for ch in t if ch.isalnum() or ch.isspace()).strip()
...
def ocr_sample_color(page, line_text, img_path):
    ...
    for i, txt in enumerate(data['text']):
        ...
        if score and score > best_score:
            best_score = score
            best_idx = i
    # if token found, sample 5x5 around token center
    if best_idx is not None:
        ...
        if med is not None:
            return rgb_to_hex_tuple(med), med
    # fallback: vertical left strip center 5x5
    cx = int((0 + int(w_img*0.15)) / 2)
    cy = int(h_img * 0.4)
    medp = median_5x5(img, cx, cy)
    if medp:
        return rgb_to_hex_tuple(medp), medp
    # final fallback: median of small center crop
    sample_box = (int(w_img*0.05), int(h_img*0.2), int(w_img*0.25), int(h_img*0.6))
...
def _box_mean_via_uniform_or_integral(arr, size=7):
    ...
    try:
        import scipy.ndimage as _nd
        return _nd.uniform_filter(arr.astype(float), size=size, mode='constant')
    except Exception:
        h,w = arr.shape
        pad = size // 2
        # integral image approach
        row_cumsum = arr.cumsum(axis=1)
...
def classify_snippet_color_convolution(page, bbox, zoom=8, kernel=7, peak_thresh=10, ratio_thresh=1.2, debug_prefix=None):
    ...
    # channel biased signals
    r = arr[:,:,0]; g = arr[:,:,1]; b = arr[:,:,2]
    red_sig = np.maximum(r - np.maximum(g,b), 0)
    green_sig = np.maximum(g - np.maximum(r,b), 0)
    # compute local means
    red_mean = _box_mean_via_uniform_or_integral(red_sig, size=kernel)
    ...
    rgb = None
    hexv = None
    ratio = (green_peak / (red_peak + 1e-6)) if red_peak > 0 else (green_peak / (1e-6))
    uncertain = False
    if green_peak > peak_thresh and green_peak >= ratio_thresh * (red_peak + 1e-6):
        # pick pixels with strong green signal
        mask = green_sig >= (0.5 * green_peak)
        sel = arr[mask]
        if sel.size:
    ...
    # small-peak ambiguity: if both peaks are small or comparable, mark uncertain
    if max(green_peak, red_peak) < max(peak_thresh, 6) or abs(green_peak - red_peak) < (peak_thresh * 0.5):
        uncertain = True
    # if still neutral, fall back to median-based check for dark/black text
    if cat == 'neutral':
        ...
        # Treat near-white median as neutral / no-color
        if sum(med) > 740:
...
def sample_phrase_bbox_color(page, spans, zoom=6, white_threshold=740, erode_iter=1, debug_prefix=None):
    ...
    try:
        ...
        return None, None
    # scale to pixels and add small margin
    ...
    if sel.size == 0:
        return None, None
    med = tuple(map(int, np.median(sel, axis=0)))
    # if median is near-white, return None
    if sum(med) > white_threshold:
        return None, None
    return rgb_to_hex_tuple(med), med
...
def _render_page_png(pdf_path, page_num, out_png):
    # use pdftocairo -singlefile to render a single page PNG
    try:
        # render at much higher DPI so tiny colored markers become easier to detect
        # use 600 DPI to amplify tiny vector markers for raster sampling
        subprocess.run(['pdftocairo', '-r', '600', '-png', '-singlefile', '-f', str(page_num), '-l', str(page_num), str(pdf_path), str(out_png.with_suffix(''))], check=True)
...
def _detect_canonical_in_pixels(arr):
    ...
    try:
        if arr is None or arr.size == 0:
            return None
        ar = arr.reshape(-1,3).astype('int')
        # compute HSV-like metrics
        arr_f = ar.astype('float32')/255.0
        mx = arr_f.max(axis=1)
        mn = arr_f.min(axis=1)
        ...
        # accept very low saturation greens as potential markers
        green_idxs = [i for i,h,s in hues if 60 <= h <= 170 and s > 0.005]
        if len(green_idxs) >= 1:
            sel = ar[green_idxs]
        ...
        for name,canon in CANONICAL.items():
            ...
            # looser threshold to pick up pale tints
            cnt = np.sum(dists < 90000)
            if cnt >= 1:
                sel = ar[dists < 90000]
                return tuple(map(int, np.median(sel, axis=0)))
        # as a last resort, if image contains any pixel with slight green bias, pick it
        biases = [(i, pix) for i,pix in enumerate(ar) if (pix[1]-pix[0])>4 and (pix[1]-pix[2])>4]
        if biases:
...
def _sample_near_token(png_path, token, side='left'):
    ...
    try:
        ...
        crop = img.crop((x0,y0,x1,y1))
        arr = np.array(crop).reshape(-1,3)
        # first try canonical detection in the crop
        try_can = _detect_canonical_in_pixels(arr)
        if try_can is not None:
            return try_can
        nonwhite = arr[(arr.sum(axis=1) < 740)]
        if nonwhite.size:
            # prefer most-saturated pixel if any
            arr_f = nonwhite.astype('float32')/255.0
            mx = arr_f.max(axis=1)
            mn = arr_f.min(axis=1)
            sat = (mx - mn) / np.where(mx == 0, 1e-6, mx)
            # also consider closeness to canonical colors
            best_score = -1.0
            best_pix = None
            for i,pix in enumerate(nonwhite):
                s = float(sat[i])
                ...
                if score > best_score:
                    best_score = score
                    best_pix = pix
            if best_pix is not None and best_score > 0.02:
...
def _sample_left_of_line(png_path, tokens, best_idx):
    ...
    try:
        ...
        cy = top_ref + int(near[0].get('height',0)) // 2
        x0 = 0
        # extend the sampling width to include potential marker columns further left
        x1 = min(img.size[0], min_left + 40)
        y0 = max(0, cy - 12)
        y1 = min(img.size[1], cy + 12)
        crop = img.crop((x0,y0,x1,y1))
        arr = np.array(crop).reshape(-1,3)
        # try blob detection on the crop first
        try:
            a = arr.reshape(-1,3)
            ...
            if mask_green.sum() > 0:
                # prefer dense small clusters rather than the global median to avoid background bias
                h_crop = y1 - y0
                w_crop = x1 - x0
                a2 = a.reshape(h_crop, w_crop, 3)
                ...
                if best_cent is not None and best_count >= 4:
                    yc, xc = best_cent
                    ylo = max(0, yc-4); yhi = min(h_crop, yc+5)
                ...
                return tuple(map(int, np.median(sel, axis=0)))
            # red candidate
            mask_red = (a[:,0].astype(int) - a[:,1].astype(int) > 20) & (a[:,0].astype(int) - a[:,2].astype(int) > 20)
            if mask_red.sum() > 0:
                sel = a[mask_red]
                return tuple(map(int, np.median(sel, axis=0)))
        except Exception:
            pass
        # If simple channel masks didn't find a blob, try density-based small-cluster search
        try:
            h_crop = y1 - y0
            w_crop = x1 - x0
            a2 = arr.reshape(h_crop, w_crop, 3)
            # mask for slight green bias and reasonably bright green channel
            mask = (a2[:,:,1].astype(int) - a2[:,:,0].astype(int) > 4) & (a2[:,:,1].astype(int) - a2[:,:,2].astype(int) > 4) & (a2[:,:,1] > 90)
            coords = np.argwhere(mask)
            if coords.size:
                # for each coord, count neighbors within a small window and pick densest
                best_count = 0
                best_cent = None
                best_cent_near_line = None
                best_count_near_line = 0
                ...
                for yx in coords:
                    ...
                    if cnt > best_count:
                        best_count = cnt
                        best_cent = (y0c, x0c)
                    # track best cluster that is near the token line
                    if abs(y0c - cy_rel) <= 12 and cnt > best_count_near_line:
                        best_count_near_line = cnt
                        best_cent_near_line = (y0c, x0c)
                chosen = None
                if best_cent_near_line is not None and best_count_near_line >= 4:
                    chosen = best_cent_near_line
                elif best_cent is not None and best_count >= 6:
                    chosen = best_cent
            ...
            pass
        # try canonical detection
        can = _detect_canonical_in_pixels(arr)
        if can is not None:
            return can
        # otherwise use the band median logic
        return _sample_left_band_median_from_png(png_path)
    except Exception:
        return None
...
def _sample_left_band_median_from_png(png_path):
    try:
        ...
        band = img.crop((0, int(h*0.15), int(w*0.25), int(h*0.85)))
        arr = np.array(band).reshape(-1,3)
        # first, try to detect canonical tint in the band
        can = _detect_canonical_in_pixels(arr)
        if can is not None:
            return can
        nonwhite_mask = (arr.sum(axis=1) < 740)
        if nonwhite_mask.sum():
            sel = arr[nonwhite_mask]
            # compute a composite score that prefers saturation and closeness to canonical colors
            arr_f = sel.astype('float32')/255.0
            mx = arr_f.max(axis=1)
            mn = arr_f.min(axis=1)
            sat = (mx - mn) / np.where(mx == 0, 1e-6, mx)
            val = mx
            mask = (sat > 0.06) & (val > 0.05)
            if mask.sum() > 8:
                sel = arr[mask]
                med = tuple(map(int, np.median(sel, axis=0)))
                return rgb_to_hex_tuple(med), med
            ...
            # try canonical detection on small crop
            can2 = _detect_canonical_in_pixels(arr2)
            if can2 is not None:
                return can2
            med2 = tuple(map(int, np.median(arr2, axis=0)))
            return med2
    except Exception:
        return None
...
def _sample_left_cluster_full(png_path, target_y=None):
    ...
    try:
        img = Image.open(str(png_path)).convert('RGB')
        w,h = img.size
        strip = np.array(img.crop((0, 0, int(w*0.25), h))).astype('int')
        h2,w2,_ = strip.shape
        a = strip.reshape(-1,3)
        if color == 'green':
        ...
        for i,(y,x) in enumerate(coords):
            if used[i]:
                continue
            # grow a local cluster
            dy = np.abs(coords[:,0] - y); dx = np.abs(coords[:,1] - x)
            sel = (dy <= 5) & (dx <= 5)
...
def run_expectation_only_qa():
    ...
    for fname, exp in loaded_expect_inner.items():
        ...
        if not candidate.exists():
            candidate = ROOT / 'data' / 'pdf_analysis' / fname
        if not candidate.exists():
        ...
        mismatches = 0
        amb_count = 0
        for phrase, expected_color in exp.items():
            total += 1
            uncertain = False
            found_in_pdf = any(phrase.lower() in (page_texts.get(p,'') or '').lower() for p in page_texts)
            found_rows = []
            found_in_rows = False
            row_cat = ''
            sampled_rgb = None
            ...
            for p, txt in page_texts.items():
                if phrase.lower() in (txt or '').lower():
                    page_hit = p
                    break
            # If phrase not found in the plain page text, try the deterministic span->conv->glyph search
            if not found_in_pdf:
                try:
                    import fitz as _fitz
                    doc_fit = _fitz.open(candidate)
                    cf = color_first_search_for_phrase(doc_fit, phrase, expected_color=expected_color, page_limit=3)
                    if cf:
                        pidx, text_cf, hex_cf, rgb_cf, page_bbox_cf, pix_bbox_cf, *cf_rest = cf
                        uncertain_cf = cf_rest[0] if cf_rest else False
                        uncertain = uncertain_cf
                        found_in_pdf = True
                        page_hit = pidx
                        sampled_rgb = rgb_cf
                        sampled_cat = map_color_to_cat(rgb_cf)
                        found_rows.append({'method': 'color_first_search', 'rgb': rgb_cf, 'hex': hex_cf, 'uncertain': uncertain_cf})
                        # if classifier matched expected color and not uncertain, mark as found_in_rows and accept
                        if sampled_cat == expected_color and not uncertain_cf:
                            found_in_rows = True
                            sampled_hex = ('#%02x%02x%02x' % sampled_rgb) if sampled_rgb else ''
                            row_cat = sampled_cat
                            continue
                except Exception:
                    pass
            if page_hit:
                # try the deterministic span->conv->glyph flow first
                try:
                    import fitz as _fitz
                    doc_fit = _fitz.open(candidate)
                    cf = color_first_search_for_phrase(doc_fit, phrase, expected_color=expected_color, page_limit=3)
                    if cf:
                        ...
                        # render page png for diagnostics
                        try:
                            ...
                            pass
                        # if the deterministic result matches expectation and is not uncertain, accept early
                        if sampled_cat == expected_color and not uncertain_cf:
                            found_in_rows = True
                            sampled_hex = ('#%02x%02x%02x' % sampled_rgb) if sampled_rgb else ''
                            row_cat = sampled_cat
                ...
                if png:
                    # token-level sampling via tesseract TSV
                    tokens = _tesseract_tokens(png)
                    target = ' '.join(ch.lower() for ch in phrase if ch.isalnum() or ch.isspace()).strip()
                    best_idx = None
                    best_score = 0
                    for i, t in enumerate(tokens):
                        ...
                        if not txt:
                            continue
                        norm = ' '.join(ch.lower() for ch in txt if ch.isalnum() or ch.isspace()).strip()
                        score = len(set(norm.split()) & set(target.split()))
                        if score > best_score:
                            best_score = score
                            best_idx = i
                    med_left = _sample_near_token(png, tokens[best_idx], side='left') if (best_idx is not None and best_score>0) else None
                    med_right = _sample_near_token(png, tokens[best_idx], side='right') if (best_idx is not None and best_score>0) else None
                    med_line = _sample_left_of_line(png, tokens, best_idx) if (best_idx is not None and best_score>0) else None
                    med_band = _sample_left_band_median_from_png(png)
                    med_cluster = _sample_left_cluster_full(png, target_y=(int(tokens[best_idx].get('top',0)) if best_idx is not None else None))
                    # choose best med by simple confidence heuristics
                    def _med_confidence(m):
                        if m is None:
                            return 0.0
                        name = map_color_to_cat(m)
                        ...
                        # but with a boosted path for canonical-proximal low-sat candidates
                        rf,gf,bf = [x/255.0 for x in m]
                        h,s,v = colorsys.rgb_to_hsv(rf,gf,bf)
                        # distance to best canonical
                        dists = {n: color_distance(m,c) for n,c in CANONICAL.items()}
                        bestd = min(dists.values())
                        # If close to a canonical color (loose threshold) and some saturation, boost
                        if bestd < 90000 and s > 0.005:
                            return 2.5 * (1.0 / (1 + bestd/30000.0)) + (s * 2.0)
                        # slight green bias (g larger than r,b) is also a useful signal
                        if (m[1] - max(m[0], m[2])) > 8:
                            return 1.5 + (s * 1.0)
                        # fallback scoring
                        return (s + 0.02) * (1.0 / (1 + bestd/20000.0)) + (0.5 if bestd < 20000 else 0.0)
                    # include cluster candidate last (it tends to be higher-precision when present)
                    # include the combined span/vector/glyph fallback as an additional candidate
                    med_combined = None
                    try:
                    ...
                    try:
                        import fitz as _fitz
                        doc = _fitz.open(candidate)
                        page_obj = doc.load_page(page_hit-1)
                        found_bbox = None
                        td = page_obj.get_text('dict')
                        for bb in td.get('blocks', []):
                            for ln2 in bb.get('lines', []):
                                lt = ''.join([s.get('text','') for s in ln2.get('spans', [])]).strip()
                                if not lt:
                                    continue
                                if phrase.lower() in lt.lower() or any(tok.lower() in phrase.lower() or phrase.lower() in tok.lower() for tok in lt.split()):
                    ...
                    for c in candidates:
                        ...
                        if sc > best_score:
                            ...
                            best = c
                    med = best
                    # Debug: for the specific 1314 PDF, record candidate confidences to help iterating heuristics
                    try:
                        if 'user_1314' in candidate.name:
                            ...
                            with open(dbg_file, 'a', newline='') as fhdbg:
                                ...
                                # also note chosen
                                wdbg.writerow([candidate.name, phrase, 'chosen', ('#%02x%02x%02x' % med) if med else '', map_color_to_cat(med) if med else '', '---'])
                    except Exception:
                        pass
                    if med:
                        sampled_rgb = med
                        ...
                        if sampled_cat == 'neutral':
                            try:
                                ...
                                if page_hit and png:
                                    ...
                                    # find token most similar to the phrase again
                                    best_idx2 = None
                                    best_score2 = 0
                                    target2 = ' '.join(ch.lower() for ch in phrase if ch.isalnum() or ch.isspace()).strip()
                                    for i, t in enumerate(tokens2):
                                        ...
                                        if norm in target2 or target2 in norm:
                                            score += 2
                                        if score and score > best_score2:
                                            best_score2 = score
                                            best_idx2 = i
                                    if best_idx2 is not None and best_score2 > 0:
                                ...
                                if diag_crop is not None:
                                    ...
                                    # compute candidate pixel metrics
                                    cands = []
                                    for pix in arr:
                                        r,g,b = map(int,pix)
                                    ...
                                    # take top 6 by combined score
                                    for cc in sorted(cands, key=lambda x: (-(x['sat']*1.0 + (max(0, (40000.0 - x['bestd'])/40000.0))*6.0)))[:6]:
                                ...
                                pass
            # if still neutral, try horizontal band scan to catch faint colored markers
            if cat == 'neutral':
                try:
                    hex3, rgb3 = None, None
                    def band_scan_color(page, line_text, img_path):
                        pix = page.get_pixmap(matrix=fitz.Matrix(2,2), alpha=False)
                        img = Image.frombytes('RGB', [pix.width, pix.height], pix.samples)
                        data = pytesseract.image_to_data(img, output_type=pytesseract.Output.DICT)
                        target = ''.join(ch.lower() for ch in line_text if ch.isalnum() or ch.isspace()).strip()
                ...
                try:
                    ...
                    def band_scan_color(page, line_text, img_path):
                        ...
                        if mask.sum() > 8:
                            ...
                            med = tuple(map(int, np.median(sel, axis=0)))
                            return rgb_to_hex_tuple(med), med
                        # fallback: median of non-white pixels
                        nonwhite = arr[(arr.sum(axis=1) < 740)]
                        if nonwhite.size:
                            med = tuple(map(int, np.median(nonwhite, axis=0)))
                            return rgb_to_hex_tuple(med), med
                        return None, None
                    hex3, rgb3 = band_scan_color(page, ln, IMG_DIR / f"{fname}.p{page_idx}.band.png")
                    if hex3 and rgb3:
                        color_hex, rgb = hex3, rgb3
                        method = 'band'
                        cat = map_color_to_cat(rgb)
                except Exception:
                    pass
            color_counts[cat] = color_counts.get(cat,0) + 1
            canonical = map_line_to_canonical(ln)
            # If we have an expectation for this filename and this line, try targeted
            # canonical/cluster/global searches to recover pale/anti-aliased markers
            try:
                exp_for_file = EXPECT.get(fname, {}) if 'EXPECT' in locals() else {}
                expected_color = None
                for ep, ec in exp_for_file.items():
                    # match by normalized/canonicalized phrase
                    if canonicalize(ep) == canonicalize(ln) or canonicalize(ep) in canonicalize(ln) or canonicalize(ln) in canonicalize(ep):
                        expected_color = ec
                        break
                if cat == 'neutral' and expected_color:
                    # render page PNG if not already
                    try:
                    ...
                    # tokenization
                    tokens = _tesseract_tokens(png)
                    # find best token for phrase
                    best_idx = None; best_score = 0
                    target = ' '.join(ch.lower() for ch in ln if ch.isalnum() or ch.isspace()).strip()
                    for i,t in enumerate(tokens):
                        txt = (t.get('text') or '').strip()
                        if not txt: continue
                        norm = ' '.join(ch.lower() for ch in txt if ch.isalnum() or ch.isspace()).strip()
                    ...
                    # try canonical near line
                    try_can = _find_canonical_near_line(png, tokens, best_idx, expected_color) if expected_color else None
                    if try_can is not None:
                        color_hex = '#%02x%02x%02x' % try_can
                        rgb = try_can
                        cat = expected_color if map_color_to_cat(try_can) == expected_color or color_distance(try_can, CANONICAL.get(expected_color)) < 120000 else map_color_to_cat(try_can)
                        method = 'canon_search'
                    # aggressive green cluster & left-strip clustering are marker-oriented; only run if markers enabled
                    if markers_enabled() and cat == 'neutral' and expected_color == 'green':
                        ag = _aggressive_green_search(png, tokens, best_idx)
                        if ag is not None and map_color_to_cat(ag) == 'green':
                            color_hex = '#%02x%02x%02x' % ag
                            rgb = ag
                            cat = 'green'
                            method = 'aggr_green'
                        ...
                        if clusters and best_idx is not None:
                            t = tokens[best_idx]
                            token_cy = int(t.get('top',0)) + int(t.get('height',0))//2
                            best_cluster = min(clusters, key=lambda c: abs(c[0] - token_cy))
                            if abs(best_cluster[0] - token_cy) <= 40 and best_cluster[3] >= 4:
                                color_hex = '#%02x%02x%02x' % best_cluster[2]
                                rgb = best_cluster[2]
                                cat = 'green'
                                method = 'strip_cluster'
                    # final global canonical search
                    if cat == 'neutral' and expected_color:
                        token_cy = None
                        if best_idx is not None:
                            t = tokens[best_idx]
                            token_cy = int(t.get('top',0)) + int(t.get('height',0))//2
                        g = _global_canonical_search(png, token_cy, expected_color)
                        if g is not None:
                            color_hex = '#%02x%02x%02x' % g
                            rgb = g
                            cat = map_color_to_cat(g)
                            method = 'global_canon'
            except Exception:
                pass
            ...
            # only append row_records for known canonical factors or non-neutral colors
            if canonical in known_keys or cat != 'neutral' or '$' in ln or 'inq' in ln.lower():
                row_records.append({
                    'filename': fname,
                    'source': src,
                    'record_id': record_id,
        ...
        # include EXPECT phrases if present
        try:
        ...
        for phrase in targeted_phrases:
            if phrase.lower() in full_text.lower():
                # find page containing the phrase and sample color
                for p in range(len(doc)):
                    ...
                    if phrase.lower() in ptext.lower():
                        ...
                        try:
                            ...
                            if med is not None:
                                hex3 = '#%02x%02x%02x' % med
                                rgb3 = med
                        except Exception:
                            hex3, rgb3 = None, None
                        cat = map_color_to_cat(rgb3) if rgb3 else 'neutral'
                        ...
                        break
        # sample record-level colors for important fields (credit score, payments, etc.) when available
        def sample_label_color(doc, label_regex_list):
            # look for any label in the document and return sampled RGB using band_scan_color_for_phrase
            for label in label_regex_list:
        ...
        # compute wide row
        wide_row = {'filename': fname, 'source': src, 'record_id': record_id}
        # default factor columns (only known canonical keys)
        for k,v in factor_map.items():
            wide_row[k] = v
        wide_row['other_factors'] = ';'.join([f"{k}:{c}" for k,c in other_factors[:80]])
        # counts per color
        wide_row['count_red'] = color_counts.get('red',0)
        ...
        # record-level numeric fields
        wide_row['credit_score'] = rec.get('credit_score','')
        wide_row['age'] = rec.get('age','')
        # attempt to sample credit score color
        cs_med = sample_label_color(doc, ['Credit Score', r'\b\d{3}\b'])
        if cs_med:
        ...
        # payment sampling
        pay_med = sample_label_color(doc, ['Payments?\b', 'Payment\b'])
        if pay_med:
        ...
        # record-level numeric fields parsed earlier
        wide_row['monthly_payment'] = rec.get('monthly_payment','')
        ...
        wide_row['late_pays_prior'] = rec.get('late_pays_prior','')        # extract open/closed counts heuristics
        m_open_closed = re.search(r"(\d+) open.*?(\d+) closed", full_text, re.I|re.S)
        if m_open_closed:
            wide_row['num_open'] = int(m_open_closed.group(1))
            wide_row['num_closed'] = int(m_open_closed.group(2))
        # quick collection counts
        wide_row['has_collections'] = 1 if 'collection' in full_text.lower() else 0
# extract open/closed collection counts
    m_open_col = re.search(r"(\d+) open.*collection", full_text, re.I)
    if m_open_col:
    ...
    wide_row['rev_balance_total'] = None
    wide_row['installment_balance_total'] = None
    wide_row['rev_limit_total'] = None
    wide_row['monthly_income'] = None
    wide_row['monthly_payment'] = None
    # try to parse balances and incomes
    m_rev = re.search(r"rev.*?balance.*?\$?([0-9,]+)", full_text, re.I)
    if m_rev:
    ...
    # try to parse monthly income / monthly payments
    m_income = re.search(r"\$(\d{1,3}(?:,\d{3})*)(?:\s*/\s*mo|/mo| per month)", full_text, re.I)
    if m_income:
    ...
    if wide_row['rev_balance_total'] and wide_row.get('rev_limit_total'):
        try:
            wide_row['rev_utilization'] = wide_row['rev_balance_total'] / wide_row['rev_limit_total']
        except Exception:
            wide_row['rev_utilization'] = None
    else:
        wide_row['rev_utilization'] = None
    if wide_row['installment_balance_total'] and wide_row.get('monthly_income'):
        try:
            wide_row['installment_balance_to_income'] = wide_row['installment_balance_total'] / (wide_row['monthly_income'] * 12)
        except Exception:
            wide_row['installment_balance_to_income'] = None
    else:
        wide_row['installment_balance_to_income'] = None
    if wide_row.get('monthly_payment') and wide_row.get('monthly_income'):
        try:
    ...
    wide_row['public_records'] = pr_count
    wide_row['public_record_note'] = pr_note
    # num_90d_plus_lates
    m_90 = re.findall(r'(\d+)\s*\b90d\b|90\+\s*d', full_text, re.I)
    wide_row['num_90d_plus_lates'] = sum(int(x) for x in re.findall(r'(\d+)\s*90\+|90d', full_text, re.I) if x.isdigit()) if re.findall(r'(\d+)\s*90', full_text, re.I) else 0
    wide_row['num_90d_plus_lates'] = wide_row.get('num_90d_plus_lates', 0) + (1 if '90d' in full_text and 'late' in full_text else 0)
    # other boolean heuristics (derived from counts when possible)
    out['has_public_record'] = 1 if out.get('public_records',0) > 0 or out.get('public_record_note') else 0
    out['has_collections'] = 1 if (out.get('collections_open',0) + out.get('collections_closed',0)) > 0 else 0
    # if a doc is provided, try to extract table-like numeric fields (installment/revolving) from page text and fall back to OCR if needed
    try:
        if doc is not None:
            prev_ln = ''
            for p in range(len(doc)):
                ...
                for b in td.get('blocks', []):
                    for ln in b.get('lines', []):
                        ...
                        try:
                            # candidate matches on prev/current/combined
                            cnt_comb, amt_comb = parse_count_amount_pair(combined)
                            ...
                            def is_revolv(s):
                                s = (s or '').lower()
                                return 'revolv' in s or 'rev' in s or 'revolving' in s or 'revolving' in s
                            def is_loc(s):
                                s = (s or '').lower()
                            ...
                            if cnt_line is not None or amt_line is not None:
                                if is_install(prev_ln):
                                    ...
                                    assigned = True
                                # if not attached to prev, attach to current if its label matches
                                if not assigned:
                                    if is_install(ln_text):
                                        ...
                                        if out.get('installment_open_total') in (None,'') and amt_line is not None:
                                            out['installment_open_total'] = int(amt_line)
                                        assigned = True
                                    elif is_revolv(ln_text) or is_loc(ln_text):
                                        if out.get('revolving_open_count') in (None,'') and cnt_line is not None:
                                            out['revolving_open_count'] = int(cnt_line)
                                        if out.get('revolving_open_total') in (None,'') and amt_line is not None:
                                            out['revolving_open_total'] = int(amt_line)
                                        assigned = True
                            # else consider prev numeric or combined
                            if (not assigned) and (cnt_prev is not None or amt_prev is not None):
                                if is_install(prev_ln):
                                ...
                                assigned = True
                            # fallback: combined detection but prefer prev label if both have labels
                            if (not assigned) and (cnt_comb is not None or amt_comb is not None):
                            ...
                            pass
                        # extra parsing: detect inquiries, collections, late pays, and payments from nearby table lines
                        low_ln = ln_text.lower()
                        # Inquiries (Last 6 Months): single number often on its own line
                        if 'inquiri' in low_ln or 'inquir' in low_ln or 'inquires' in low_ln:
                            # avoid capturing the timeframe 'Last 6 Months' (6) - prefer an adjacent actual count
                            if not re.search(r"last\s*\d+\s*months?", ln_text, re.I):
                                ...
                                if m_in:
                                    ...
                                    # else try previous/next numeric
                            else:
                                # try previous or following numeric; previous may contain a number
                                m_prev = re.search(r"(\d+)", prev_ln)
                                if m_prev:
                                    out['inquiries_6mo'] = int(m_prev.group(1))
                                else:
                                    # try to find the number on the following line using combined detection
                                    if cnt_line is not None:
                                        out['inquiries_6mo'] = int(cnt_line)
                        # Collections (Open/Closed): '0 / 0' may be on same or next line
                        if 'collections' in low_ln:
                            m_col = re.search(r"(\d+)\s*/\s*(\d+)", ln_text)
                            if m_col:
                                out['collections_open'] = int(m_col.group(1))
                                out['collections_closed'] = int(m_col.group(2))
                            else:
                                # try next/prev line via combined
                                if cnt_line is not None and amt_line is not None:
                                    out['collections_open'] = int(cnt_line)
                                    out['collections_closed'] = int(amt_line)
                        # Late Pays (Last 2/2+ Years) -> look for '7 / 0' pair on same or next line
                        if 'late pays' in low_ln:
                        ...
                        # Payments: look for dollar amount on same or adjacent lines
                        if 'payments' in low_ln or 'payment' in low_ln:
                        ...
                        prev_ln = ln_text
                # if still missing, try OCR on rendered page
                if (out.get('installment_open_count') in (None,'') or out.get('installment_open_total') in (None,'')) or (out.get('revolving_open_count') in (None,'') or out.get('revolving_open_total') in (None,'')):
                    try:
                        import pytesseract
                        ...
                        pass
            # finally, populate credit factors using the right-column extractor
            try:
                out['credit_factors'] = extract_credit_factors_from_doc(doc)
            except Exception:
                out['credit_factors'] = out.get('credit_factors', [])
            # post-page scan heuristics: if we still don't have inquiries or late pays, try page-text searches
            try:
                ...
                if out.get('inquiries_6mo') in (None,''):
                    for m in re.finditer(r"Inquir(?:y|ies|es)?", page_text, re.I):
                        ...
                        for ln in snippet.splitlines():
                            ...
                            if not ln_strip:
                                continue
                            if re.search(r"months?|mo", ln_strip, re.I):
                                # skip lines that are just the label with 'Last 6 Months'
                                continue
                            mnum = re.search(r"\b(\d{1,3})\b", ln_strip)
                            if mnum:
                                out['inquiries_6mo'] = int(mnum.group(1))
                                break
                        if out.get('inquiries_6mo') not in (None,''):
                            break
                # Late Pays: find a numeric 'x / y' following the label
                # prefer pairs on the following line to avoid matching the '(Last 2/2+ Years)' parenthetical
                found = False
                for m in re.finditer(r"Late Pays", page_text, re.I):
                    ...
                    if found:
                        break
                # Credit Card Open Totals (No Retail): inspect account table blocks and consider their final 'total' line
                try:
                    cc_found = None
                    for p in range(len(doc)):
                        td = doc.load_page(p).get_text('dict')
                        ...
                        for bi, b in enumerate(blocks):
                            ...
                            # Heuristic: block that contains 'revolving accounts' or 'revolving accounts (open)' likely precedes credit card totals
                            if 'revolving accounts' not in low_block and 'revolving accounts (open)' not in low_block:
                                continue
                            # scan following blocks for a block containing 'total' (e.g., 'Credit Card Open Totals')
                            total_block = None
                            for j in range(bi+1, min(bi+30, len(blocks))):
                                b2 = blocks[j]
                            ...
                            if not total_block:
                                continue
                            # get the last line that contains 'total' or the first line of total_block
                            total_lines = [ln for ln in total_block.get('lines', []) if 'total' in ''.join([s.get('text','') for s in ln.get('spans', [])]).lower()]
                            if total_lines:
                                ln = total_lines[-1]
                            else:
                                # fallback to first meaningful line
                                ln = total_block.get('lines', [])[0]
                            line_text = ''.join([s.get('text','') for s in ln.get('spans', [])]).strip()
                            ...
                            # look for dollar amounts and percent in block lines
                            lines = [lt.strip() for lt in block_text_full.splitlines() if lt.strip()]
                            # simple heuristics based on observed layout
                            amounts = []
                            perc = None
                            for ln_txt in lines:
                            ...
                            if amounts:
                                ...
                                if len(amounts) >= 3:
                                    pay = amounts[2]
                                util = perc
                            # determine color: prefer span color of the label line then convolution fallback
                        ...
                        if cc_found:
                            break
                    if cc_found:
                        out['credit_card_open_totals_no_retail'] = cc_found
                except Exception:
                    pass
                # if not found, do not fallback to parenthetical pair (avoid capturing '2/2' from label)
                # monthly payment: fallback to first $NNN/mo on the page if not found
                if out.get('monthly_payment') in (None,''):
        ...
        pass
    # ensure numeric fields are present as empty values if not found
    out['installment_open_count'] = out.get('installment_open_count', '')
...
def canonicalize(s):
    ...
    s = re.sub(r"\$[0-9,]+", " ", s)
    s = re.sub(r"^\s*\d+\s+", " ", s)
    # keep numeric ranges (e.g., '4-6', '6-12') so mapping rules can distinguish buckets
...
def parse_public_records(full_text: str):
    ...
    # capture bankruptcy/other public-record notes
...
def load_expectations_from_dir(dpath):
    ...
    Looks for files with a 'PDF file:' line and a 'Credit Factors:' section. If the explicit
    filename referenced by the file is missing, try to find a best-match by user id prefix
            ...
            for ln in fh:
                ...
                if ln.lower().startswith('pdf file:'):
                    try:
                        ...
                        if not ((ROOT / 'data' / 'pdf_analysis' / cur_pdf).exists() or (PDF_DIR / cur_pdf).exists()):
                ...
                if in_cf:
                    ...
                    if m and cur_pdf:
                        ...
                        # stop section on blank line or non-list line
                        if ln.strip() == '':
                            in_cf = False
    return out
...
def map_line_to_canonical(line):
    ...
    for rx, key in MAPPING_RULES:
        if re.search(rx, s):
            return key
    # fallback: use normalized text as key
...
def normalize_factors(raw_factors):
    ...
    This removes isolated dollar-only tokens, merges category+value pairs, removes noise
    like 'Payment Resp' and dedupes by canonical key preferring highest severity color.
    """
    out = []
    i = 0
    import re
    ...
    while i < len(raw_factors):
        ...
        if i+1 < len(raw_factors):
            ...
            if cnt is not None or amt is not None:
                count = cnt
                total = amt
                if chosen_color in (None,'neutral'):
                ...
                continue
        # If the current text itself encodes count/total
        cnt2, amt2 = parse_count_amount_pair(txt)
        ...
        if m3 and ('rev' in txt.lower() or 'late' in txt.lower() or 'lates' in txt.lower()):
            ...
            label = m3.group(0).strip()  # keep original including count
            canonical = map_line_to_canonical(m3.group(2).strip())
            out.append({'label': label, 'canonical': canonical, 'count': count, 'total': None, 'color': chosen_color or 'neutral', 'hex': chosen_hex or ''})
            i += 1
            continue
        # default: keep as label-only factor
        out.append({'label': txt, 'canonical': canonical, 'count': None, 'total': None, 'color': chosen_color or 'neutral', 'hex': chosen_hex or ''})
        i += 1
    ...
    for f in out:
        ...
        if prev.get('count') is None and f.get('count') is not None:
            prev['count'] = f['count']
            prev['label'] = f.get('label', prev.get('label'))
        if prev.get('total') is None and f.get('total') is not None:
            prev['total'] = f['total']
        # choose highest severity color
        if prio.get(f.get('color'),0) > prio.get(prev.get('color'),0):
            prev['color'] = f.get('color')
            prev['hex'] = f.get('hex')
    # produce simplified output and filter out entries we consider non-factors
    simplified = []
    for k in order:
        f = merged[k]
        if not f.get('label'): continue
        # skip 'past due' generic and 'payment resp' if still present
        if f['label'].strip().lower() in ('past due','payment resp'):
            continue
        simplified.append({'factor': f['label'], 'color': f.get('color') or 'neutral'})
    ...
    for sf in simplified:
        ...
        for k,v in overrides.items():
            if k in low:
                sf['color'] = v
    # ensure leading-count rev-late phrases are marked red, and other re-lates with counts are red
    for sf in simplified:
        low = sf['factor'].lower()
        # mark rev lates with any count >=4 as red
        m = re.match(r"^(\d+)\s+rev(?:o)?\s*late[s]?", low)
        if m:
            try:
                cnt = int(m.group(1))
                if cnt >= 4:
                ...
                pass
        if low.startswith('1 rev late') or 'rev late in 0-3' in low:
            sf['color'] = 'red'
...
def extract_credit_factors_from_doc(doc, page_limit=None):
    ...
    while i < len(candidates):
        ...
        txt = text.strip()
        # if this line is just a dollar amount or money+percent and next line looks like collection/collection(s), merge
        if re.match(r"^\$[\d,]+(\s*\d+%|\s*%?)$", txt) and i + 1 < len(candidates):
    ...
    for p_idx, text, spans, x0 in merged_candidates:
        ...
        try:
            ...
            line_bbox = None
        # Always use span color as primary
        hexv, rgb = span_color_hex(spans)
        # Debug: print detailed span info for the target phrase
        if text and '3 re lates in 6-12' in text.lower():
            try:
                ...
                for s in spans:
                    ...
                    # attempt to normalize span color to rgb for inspection
                    try:
                        rgb_span = None
                        if isinstance(s_col, (tuple, list)):
                        ...
                        pass
                print('  span_color_hex ->', hexv, rgb, 'mapped cat:', map_color_to_cat(rgb) if rgb else None)
            except Exception:
                pass
        # If span color is missing or ambiguous, use convolution fallback
        if rgb is None or (hexv is None and line_bbox is not None):
            try:
        ...
        # DEBUG: print final mapping for the target phrase
        if text and '3 re lates in 6-12' in text.lower():
...
def _detect_canonical_in_pixels(arr):
    ...
    try:
        ...
        for name,canon in CANONICAL.items():
            dists = np.sum((ar - np.array(canon))**2, axis=1)
            # looser threshold to pick up pale tints or anti-aliased pixels
            cnt = np.sum(dists < 90000)
            if cnt >= 1:
                sel = ar[dists < 90000]
                return tuple(map(int, np.median(sel, axis=0)))
            ...
            if coords.size:
                ...
                for yx in coords:
                    ...
                    if cnt > best_count:
                        ...
                        best_cent = (y0c, x0c)
                    # track best cluster that is near the token line
                    if abs(y0c - cy_rel) <= 12 and cnt > best_count_near:
                        best_count_near = cnt
                        best_cent_near = (y0c, x0c)
                chosen = None
                if best_cent_near is not None and best_count_near >= 4:
                    chosen = best_cent_near
                elif best_cent is not None and best_count >= 6:
                    chosen = best_cent
                if chosen is not None:
                    yc, xc = chosen
                    ylo = max(0, yc-4); yhi = min(h2, yc+5)
...
def _sample_left_cluster_full(png_path, target_y=None):
    ...
    try:
        ...
        if coords.size == 0:
            return None
        # prefer clusters near target_y (if provided), where target_y is page coordinate
        if target_y is not None:
            # convert target_y (page coord) into band-relative coord
            band_y0 = int(h*0.15)
            cy_rel = target_y - band_y0
        else:
            cy_rel = None
        best_count = 0
        best_cent = None
...
def _find_canonical_near_line(png_path, tokens, best_idx, expected_color):
    ...
    try:
        ...
        near = [t for t in tokens if abs(int(t.get('top',0)) - top_ref) <= 8]
        if not near:
            # fallback to center band
            cy = top_ref + int(near[0].get('height',0)) // 2
            x0 = 0
            x1 = int(png.size[0] * 0.25)
            y0 = max(0, cy - 12)
            y1 = min(png.size[1], cy + 12)
        else:
            min_left = min(int(t.get('left',0)) for t in near)
            cy = top_ref + int(near[0].get('height',0)) // 2
            x0 = 0
            # widen x search to capture markers that are slightly further left/right of tokens
            x1 = min(png.size[0], min_left + 80)
            # widen y band to be more tolerant of vertical offsets
            y0 = max(0, cy - 20)
        ...
        if arr.size == 0:
            return None
        # compute distance to the expected canonical
        canon = CANONICAL.get(expected_color)
        if not canon:
            return None
        dists = np.sum((arr - np.array(canon))**2, axis=1)
        best_idx = np.argsort(dists)[:200]
        coords = [divmod(int(i), png.size[0]) for i in best_idx]
        # map coords back to original image coordinates
        best_choice = None
        best_dy = None
        for yy, xx in coords:
            orig_x = int(xx * 4 + 4 // 2)
            orig_y = int(yy * 4 + 4 // 2)
            dy = abs(orig_y - top_ref) if top_ref is not None else 0
            if best_dy is None or dy < best_dy:
                best_dy = dy
                best_choice = (orig_x, orig_y)
        if best_choice is None:
            return None
        px = img.getpixel(best_choice)
        # accept only if reasonably close to canonical
        if np.sum((np.array(px) - canon)**2) < 250000:
            return tuple(px)
        return None
    except Exception:
        return None
...
def _aggressive_green_search(png_path, tokens, best_idx):
    ...
    try:
        ...
        min_left = min(int(t.get('left',0)) for t in near)
        cy = top_ref + int(near[0].get('height',0)) // 2
        x0 = max(0, min_left - 80)
        # widen x1 to look slightly to the right of the left token edge as well
        x1 = min(img.size[0], min_left + 36)
        y0 = max(0, cy - 22)
        ...
        if mask.sum() == 0:
            return None
        # find densest cluster among mask points
        coords = np.argwhere(mask.reshape(h2,w2))
        best_count = 0
        best_cent = None
        best_count_near = 0
        ...
        for yx in coords:
            ...
            if abs(y0c - cy_rel) <= 12 and cnt > best_count_near:
                ...
                best_cent_near = (y0c, x0c)
        chosen = None
        # relax cluster size thresholds so small markers are recognized
        if best_cent_near is not None and best_count_near >= 4:
            chosen = best_cent_near
        elif best_cent is not None and best_count >= 4:
            chosen = best_cent
        if chosen is not None:
            yc, xc = chosen
            ylo = max(0, yc-4); yhi = min(h2, yc+5)
...
def _global_canonical_search(png_path, token_cy, expected_color, downscale=4, max_candidates=200):
    """Search the whole page for pixels nearest the expected canonical color, then pick
    the candidate whose vertical position is closest to token_cy. Returns RGB tuple or None."""
    try:
        img = Image.open(str(png_path)).convert('RGB')
        w,h = img.size
        # downscale for speed
        ...
        if canon is None:
            return None
        dists = np.sum((arr - canon)**2, axis=1)
        ...
        for yy, xx in coords:
            ...
            dy = abs(orig_y - token_cy) if token_cy is not None else 0
            if best_dy is None or dy < best_dy:
                best_dy = dy
                best_choice = (orig_x, orig_y)
        if best_choice is None:
...
def main():
    import csv
    rows = []
    # if PyMuPDF is not available, run the lightweight expectation-only QA and exit
    if not HAS_FITZ:
        print('PyMuPDF not available. Running expectation-only QA and exiting main.')
        run_expectation_only_qa()
        return
    # process only the PDFs already downloaded in PDF_DIR, unless expectation files point to a specific PDF
    loaded_expect = load_expectations_from_dir(ROOT / 'data' / 'pdf_analysis')
    ...
    for fname in expected_filenames:
        ...
        if p1.exists():
            ...
            continue
        p2 = ROOT / 'data' / 'pdf_analysis' / fname
        if p2.exists():
            explicit_files.append(p2)
            continue
    if explicit_files:
        pdf_files = explicit_files
    else:
        pdf_files = sorted(PDF_DIR.glob('*.pdf'))
    ...
    for pdf_path in pdf_files:
        ...
        record_id = record_id.group(1) if record_id else fname
        doc = fitz.open(str(pdf_path))
        # find candidate lines across all pages that match known factor keywords
        def find_candidate_lines(doc):
            keywords = [
            ...
            return candidates
        lines = find_candidate_lines(doc)
        full_text = '\n'.join([doc.load_page(i).get_text() for i in range(len(doc))])
        rec = extract_record_level(full_text)
        # If installment/revolving counts weren't found in the text, try OCR-based candidate line scan
        try:
        ...
        for page_idx, ln, sp in lines:
            ...
            if any(h in ln_lower for h in header_exclude):
                # but keep if it contains an explicit value like 'credit freeze: no' or if it's one of the specific fields
                if ln_lower in ('credit freeze','fraud alert','credit score','payments'):
                    pass
                else:
                    continue
            # try span color first, else OCR
            color_hex, rgb = span_color_hex(sp)
            method = 'span'
            page = doc.load_page(page_idx)
                        ...
                        if best_y is None:
                            ...
                            if med:
                                if out_img_path:
                                    ...
                                    small.save(out_img_path)
                                return med
                    hex3, rgb3 = band_scan_color(page, ln_proc, IMG_DIR / f"{fname}.p{page_idx}.band.png")
                    if hex3 and rgb3:
                        color_hex, rgb = hex3, rgb3
                        method = 'band'
                        cat = map_color_to_cat(rgb)
